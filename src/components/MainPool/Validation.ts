...
export default (
  ...
  const userPercentageOfTotal =
    userPoolData.staked > 0
      ? (userPoolData.staked / basePoolData.staked) * 100
      : null;
  ...
  const canStake =
    ...
    (basePoolData.migrationStatus == null ||
      basePoolData.migrationStatus === 0);
  ...
  const canMaxWithdraw =
    userPoolData.staked > 0 &&
    (basePoolData.migrationStatus == null ||
      basePoolData.migrationStatus === 2);
  // does user have rewards
  const canClaim =
    userPoolData.rewards.tokens > 0 &&
    (basePoolData.migrationStatus == null ||
      basePoolData.migrationStatus === 2);
  ...
  const displayYields =
    ...
    !poolClosed &&
    basePoolData.APY &&
    isFinite(basePoolData.APY.weeklyROI);

  /**
   *  Calculate ROIs from the weekly amount.
   */
  const weeklyROI =
    basePoolData.APY && !poolClosed && basePoolData.APY.weeklyROI;
  const yearlyROI = weeklyROI ? weeklyROI * 52 : null;
  let weeklyUSD = null;
  if (rewardPrice && basePoolData.APY && !poolClosed) {
    weeklyUSD =
      basePoolData.APY.rewardPerToken *
      userPoolData.staked *
      rewardPrice *
      userPoolData.multiplier!;
  }
